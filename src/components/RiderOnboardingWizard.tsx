...
import { z } from 'zod';
import CinematicFoodBackground from './CinematicFoodBackground';
    ...
    switch(currentStep) {
        ...
        return (
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input type="text" value={name} onChange={e => setName(e.target.value)} disabled={!!initialName} className="w-full px-4 py-3 rounded-xl border border-rose-500/20 bg-white/20 dark:bg-slate-900/20 backdrop-blur-md text-sm font-bold text-slate-900 dark:text-white outline-none focus:border-rose-500/50" />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-500 uppercase">Vehicle Number (if applicable)</label>
              <input type="text" value={vehicle} onChange={e => setVehicle(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-rose-500/20 bg-white/20 dark:bg-slate-900/20 backdrop-blur-md text-sm font-bold text-slate-900 dark:text-white outline-none focus:border-rose-500/50" />
            </div>
            <div className="space-y-1.5">
            ...
            <button onClick={submitProfile} disabled={isSubmitting} className="w-full py-3.5 mt-4 rounded-xl bg-gradient-to-r from-rose-500 to-orange-500 text-white font-bold text-sm shadow-md flex justify-center items-center gap-2">
        ...
        if (verificationStatus?.dlApproved) {
          return (
             <div className="py-12 flex flex-col items-center justify-center text-center space-y-4">
                ...
                <button onClick={handleNext} className="w-full py-3.5 mt-4 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold text-sm">Continue</button>
             </div>
          );
        }
        return (
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-500 uppercase">DL Number</label>
              <input type="text" value={dlNumber} onChange={e => setDlNumber(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-rose-500/20 bg-white/20 dark:bg-slate-900/20 backdrop-blur-md text-sm font-bold text-slate-900 dark:text-white outline-none focus:border-rose-500/50" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-500 uppercase">Date of Birth</label>
              <input type="date" value={dob} onChange={e => setDob(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-rose-500/20 bg-white/20 dark:bg-slate-900/20 backdrop-blur-md text-sm font-bold text-slate-900 dark:text-white outline-none focus:border-rose-500/50" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-500 uppercase">Upload DL Image</label>
              <DocumentUploadField value={dlDoc} onChange={setDlDoc} docType="DRIVING_LICENSE" placeholder="Upload Front of DL" />
            </div>
            <button onClick={submitDL} disabled={isSubmitting} className="w-full py-3.5 mt-4 rounded-xl bg-gradient-to-r from-rose-500 to-orange-500 text-white font-bold text-sm shadow-md flex justify-center items-center gap-2">
              {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <ChevronRight className="w-4 h-4" />}
              Verify License
            </button>
          </div>
      ...
      case 2:
        ...
        return (
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-500 uppercase">Registration Number (Plate)</label>
              <input type="text" value={rcNumber} onChange={e => setRcNumber(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-rose-500/20 bg-white/20 dark:bg-slate-900/20 backdrop-blur-md text-sm font-bold text-slate-900 dark:text-white outline-none focus:border-rose-500/50" />
            </div>
            <div className="space-y-1.5">
        ...
        return (
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input type="text" value={bankAccount} onChange={e => setBankAccount(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-rose-500/20 bg-white/20 dark:bg-slate-900/20 backdrop-blur-md text-sm font-bold text-slate-900 dark:text-white outline-none focus:border-rose-500/50" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-500 uppercase">IFSC Code</label>
              <input type="text" value={ifsc} onChange={e => setIfsc(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-rose-500/20 bg-white/20 dark:bg-slate-900/20 backdrop-blur-md text-sm font-bold text-slate-900 dark:text-white outline-none focus:border-rose-500/50" />
            </div>
            <button onClick={submitBank} disabled={isSubmitting} className="w-full py-3.5 mt-4 rounded-xl bg-gradient-to-r from-rose-500 to-orange-500 text-white font-bold text-sm shadow-md flex justify-center items-center gap-2">
              {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <ChevronRight className="w-4 h-4" />}
              Initiate Penny Drop
            </button>
          </div>
      ...
      case 4:
        return (
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-500 uppercase">Upload Selfie</label>
              <DocumentUploadField value={selfieDoc} onChange={setSelfieDoc} docType="SELFIE" placeholder="Take a clear selfie" />
            </div>
            <button onClick={submitSelfie} disabled={isSubmitting} className="w-full py-3.5 mt-4 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-bold text-sm shadow-md flex justify-center items-center gap-2">
