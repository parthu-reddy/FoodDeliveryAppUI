    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-[#0f111a] text-slate-800 dark:text-[#f0ede6] p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button 
          ...
          className="bg-white/20 dark:bg-slate-900/40 hover:bg-slate-200 dark:hover:bg-slate-800 px-4 py-2 rounded-xl text-sm font-bold border border-slate-200 dark:border-slate-800 transition-colors"
        >
          Refresh
        </button>
      </div>
            ...
            {pendingPayouts.map((account) => (
              <div key={account.ownerId} className="bg-white/20 dark:bg-slate-900/40 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col">
                ...
                <button
                  ...
                  className="w-full py-4 rounded-2xl bg-emerald-500 hover:bg-emerald-600 disabled:bg-emerald-500/50 text-white font-bold text-lg shadow-lg shadow-emerald-500/30 transition-all flex justify-center items-center gap-2"
                >
                  {settling === account.ownerId ? (
                    <><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> Settling...</>
                  ) : (
