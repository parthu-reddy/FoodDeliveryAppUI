...
export default function BrandMasterMenu({ brandId, onRefresh }: BrandMasterMenuProps) {
  ...
  const handleCreateCategory = async (e: React.FormEvent) => {
    ...
    try {
      await apiPost('/api/v1/categories', { name: newCatName, description: newCatDesc });
  ...
  return (
    <div className="space-y-6 mt-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        ...
        <button onClick={() => setIsAddingCategory(true)} className="flex items-center gap-1 bg-rose-500 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-rose-600 transition-colors shadow-sm shadow-rose-500/20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div className="flex items-center justify-center min-h-[80px]">
                    {addingItemToCatId === cat.id ? (
                        <div className="w-full bg-orange-500/5 border border-orange-500/20 p-3 rounded-xl animate-fade-in shadow-sm shadow-orange-500/5">
                            <h6 className="font-bold text-[11px] uppercase tracking-wider text-orange-600 dark:text-orange-400 mb-2">New {cat.name} Item</h6>
                            <form onSubmit={(e) => handleCreateMaster(e, cat.id)} className="space-y-2">
                                <input required placeholder="Item Name" value={mName} onChange={e=>setMName(e.target.value)} className="w-full bg-white dark:bg-slate-950 border border-orange-500/20 rounded-lg px-2 py-1.5 text-xs font-bold dark:text-[#f0ede6]" />
                                <div className="flex gap-2">
                                  <input required type="number" step="0.01" min="0" placeholder="Base Price" value={mPrice} onChange={e=>setMPrice(e.target.value)} className="w-full bg-white dark:bg-slate-950 border border-orange-500/20 rounded-lg px-2 py-1.5 text-xs font-bold dark:text-[#f0ede6]" />
                                  <input required type="number" min="1" placeholder="Prep (mins)" value={mPrepTime} onChange={e=>setMPrepTime(e.target.value)} className="w-full bg-white dark:bg-slate-950 border border-orange-500/20 rounded-lg px-2 py-1.5 text-xs font-bold dark:text-[#f0ede6]" />
                                </div>
                                <input required placeholder="Description" value={mDesc} onChange={e=>setMDesc(e.target.value)} className="w-full bg-white dark:bg-slate-950 border border-orange-500/20 rounded-lg px-2 py-1.5 text-xs dark:text-[#f0ede6]" />
                                <div className="flex gap-2 pt-1">
                                    <button type="button" onClick={() => setAddingItemToCatId(null)} className="flex-1 py-1.5 rounded border border-orange-500/20 text-[10px] font-bold text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">Cancel</button>
                                    <button type="submit" className="flex-1 py-1.5 bg-orange-500 text-white text-[10px] font-bold rounded hover:bg-orange-600">Save Item</button>
                                </div>
                            </form>
                        </div>
                    ) : (
                        <button onClick={() => openAddItem(cat.id)} className="w-full h-full min-h-[80px] border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl flex flex-col items-center justify-center gap-2 text-slate-400 hover:text-orange-500 hover:border-orange-500/50 hover:bg-orange-500/5 transition-all">
