...
import { 
  Bike, DollarSign, Check, ShieldAlert, Store, Sun, Moon,
  AlertCircle, User, ArrowLeft, X, MapPinOff
} from 'lucide-react';
...
import { DeliveryAvailableJobs } from './DeliveryAvailableJobs';
...
export default function DeliveryDashboard({
  ...
  const handleCompleteDelivery = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsUpdatingDelivery(false);
      onUpdateOrderStatus(currentJob.id, previousStatus, currentJob.deliveryStatus); 
      setOtpError(e.response?.data?.message || "Failed to verify Delivery OTP.");
    }
  };
    ...
    <div className="flex-1 w-full flex flex-col max-w-3xl mx-auto min-h-0 relative z-0">
      ...
      {view === 'settings' ? (
        ...
        <>
          ...
          <Modal isOpen={showPermissionsPrompt} onClose={() => setShowPermissionsPrompt(false)} size="sm">
          ...
          <Modal isOpen={showProfileRequiredPrompt} onClose={() => setShowProfileRequiredPrompt(false)} size="sm">
            <div className="p-8 pb-6 flex flex-col items-center text-center">
