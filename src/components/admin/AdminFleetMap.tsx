    ...
    const fetchData = async () => {
      try {
        const [resOutlets, resDrivers, resCustomers] = await Promise.all([
          (identityApi.get as any)('/api/v1/internal/admin/restaurants/all-with-location'),
          (identityApi.get as any)('/api/v1/internal/admin/delivery/drivers/all-with-location'),
          (identityApi.get as any)('/api/v1/internal/admin/customers/addresses')
        ]);
    ...
    const renderMarkers = () => {
      ...
      restaurants.forEach(r => {
        if (r.lat && r.lng && r.lat !== 0 && r.lng !== 0) {
          ...
          if ((r as any).phone || (r as any).phoneNumber) {
            el.title = `Phone: ${(r as any).phone || (r as any).phoneNumber}`;
          }
      ...
      customers.forEach(c => {
        if (c.latitude && c.longitude && c.latitude !== 0 && c.longitude !== 0) {
          ...
          if ((c as any).phone || (c as any).phoneNumber) {
            el.title = `Phone: ${(c as any).phone || (c as any).phoneNumber}`;
          }
