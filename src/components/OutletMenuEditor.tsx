...
export default function OutletMenuEditor({ restaurantId, brandId, menuList, onRefresh }: OutletMenuEditorProps) {
        ...
        const updatedCategories = await Promise.all(
          data.data.map(async (cat: any) => {
            ...
            try {
                ...
                const oRes = await apiGet(`/api/v1/outlets/${selectedOutlet}/categories/${cat.id}/timings`);
            ...
            try {
                ...
                const bRes = await apiGet(`/api/v1/brands/${brandId}/categories/${cat.id}/timings`);
  ...
  const handleSaveTimings = async (catId: string) => {
    try {
        ...
        await apiPost(`/api/v1/outlets/${selectedOutlet}/categories/timings`, payload);
            ...
            <div key={cat.id} className="bg-white/20 dark:bg-slate-900/20 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
              ...
              <div className="p-4 bg-white/10 dark:bg-slate-950/20">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {/* Add Outlet Item Button (inside category) */}
                  <div className="flex items-center justify-center min-h-[100px]">
                    {addingItemToCatId === cat.id ? (
                        <div className="w-full bg-emerald-500/5 border border-emerald-500/20 p-4 rounded-2xl animate-fade-in shadow-sm shadow-emerald-500/5">
                            <h6 className="font-bold text-[11px] uppercase tracking-wider text-emerald-600 dark:text-emerald-400 mb-3">New Outlet Item</h6>
                            <form onSubmit={(e) => handleCreateOutletItem(e, cat.id)} className="space-y-3">
                                <div className="grid grid-cols-3 gap-3">
                                    <input required placeholder="Item Name" value={mName} onChange={e=>setMName(e.target.value)} className="w-full bg-white dark:bg-slate-950 border border-emerald-500/20 rounded-lg px-2 py-1.5 text-xs font-bold dark:text-[#f0ede6]" />
                                    <input required type="number" step="0.01" min="0" placeholder="Master Price" value={mPrice} onChange={e=>setMPrice(e.target.value)} className="w-full bg-white dark:bg-slate-950 border border-emerald-500/20 rounded-lg px-2 py-1.5 text-xs font-bold dark:text-[#f0ede6]" />
                                    <input required type="number" min="1" placeholder="Master Prep Time" value={mPrepTime} onChange={e=>setMPrepTime(e.target.value)} className="w-full bg-white dark:bg-slate-950 border border-emerald-500/20 rounded-lg px-2 py-1.5 text-xs font-bold dark:text-[#f0ede6]" />
                                </div>
                                <input required placeholder="Description" value={mDesc} onChange={e=>setMDesc(e.target.value)} className="w-full bg-white dark:bg-slate-950 border border-emerald-500/20 rounded-lg px-2 py-1.5 text-xs dark:text-[#f0ede6]" />
                                <div className="grid grid-cols-3 gap-3">
                                    <input type="number" step="0.01" min="0" placeholder="Override Price" value={oPrice} onChange={e=>setOPrice(e.target.value)} className="w-full bg-white dark:bg-slate-950 border border-emerald-500/20 rounded-lg px-2 py-1.5 text-xs font-bold dark:text-[#f0ede6]" />
                                    <input type="number" min="1" placeholder="Override Prep" value={oPrepTime} onChange={e=>setOPrepTime(e.target.value)} className="w-full bg-white dark:bg-slate-950 border border-emerald-500/20 rounded-lg px-2 py-1.5 text-xs font-bold dark:text-[#f0ede6]" />
                                    <select value={oActive ? "true" : "false"} onChange={e=>setOActive(e.target.value === "true")} className="w-full bg-white dark:bg-slate-950 border border-emerald-500/20 rounded-lg px-2 py-1.5 text-xs font-bold dark:text-[#f0ede6]">
                                        <option value="true">Active</option>
                                        <option value="false">Inactive</option>
                                    </select>
                                </div>
                                <div className="flex gap-2 pt-1">
                                    <button type="button" onClick={() => setAddingItemToCatId(null)} className="flex-1 py-1.5 rounded-lg border border-emerald-500/20 text-[10px] font-bold text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">Cancel</button>
                                    <button type="submit" className="flex-1 py-1.5 bg-emerald-500 text-white text-[10px] font-bold rounded-lg hover:bg-emerald-600">Save Item</button>
                                </div>
                            </form>
                        </div>
                    ) : (
                        <button onClick={() => openAddOutletItem(cat.id)} className="w-full h-full min-h-[100px] border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl flex flex-col items-center justify-center gap-2 text-slate-400 hover:text-emerald-500 hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-all">
                            <Plus className="w-5 h-5" />
                            <span className="text-[10px] font-bold uppercase tracking-wider">Add {cat.name} Item</span>
                        </button>
                    )}
                  </div>
                </div>
