...
import { Button, Select, Spinner, StatusPill, Surface } from '@shared/ui';
...
export default function RefundQueue() {
  ...
  const getStatusBadge = (status?: string) => {
    const { label, tone } = refundTicketStatus(status);
  ...
  return (
    <div className="flex h-full bg-slate-50 dark:bg-[#0f111a] text-slate-800 dark:text-[#f0ede6]">
      <div className="flex-1 p-6 flex flex-col">
          ...
          <Surface variant="glass-overlay" elevation={4} radius="xl" className="p-4 mb-6 flex items-center gap-4">
              <div>
                 ...
                 <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700/50">
                       {tickets.map((t) => (
                          <tr 
                             key={t.id} 
                             className={`hover:bg-slate-50 dark:hover:bg-slate-800/30 cursor-pointer transition-colors ${selectedTicket?.id === t.id ? 'bg-rose-50 dark:bg-rose-900/20' : ''}`}
                             onClick={() => {
                                 setSelectedTicket(t);
                                 setApproved(true);
                                 setNotes("");
                         ...
                         <div className="space-y-4">
                             <div className="grid grid-cols-2 gap-2">
                                 <button 
                                     ...
                                     className={`p-2 rounded-lg border-2 text-sm font-bold transition ${approved ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-slate-200 text-slate-500'}`}
                                 >
                                     Approve
                                 </button>
                                 <button 
                                     onClick={() => setApproved(false)}
                                     className={`p-2 rounded-lg border-2 text-sm font-bold transition ${!approved ? 'border-rose-500 bg-rose-50 text-rose-700' : 'border-slate-200 text-slate-500'}`}
                                 >
                                     Reject
                                 </button>
                             </div>
                             ...
                             <div>
                                 ...
                                 <textarea 
                                     className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:ring-rose-500"
                                     rows={3}
