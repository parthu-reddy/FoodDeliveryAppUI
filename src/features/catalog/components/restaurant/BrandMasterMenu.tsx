...
const BrandMasterMenu = React.memo(function BrandMasterMenu({ brandId, onRefresh }: BrandMasterMenuProps) {
 ...
 <Button onClick={() => { setIsAddingItem(true); setIsAddingCategory(false); setMName(''); setMPrice(''); setMPrepTime('15'); setMDesc(''); setMCatId(''); }} variant="warning" className="shadow-sm shadow-amber-500/20">
 ...
 {isAddingCategory && (
 <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-md border border-rose-500/20 dark:border-rose-500/30 p-4 rounded-2xl shadow-sm">
 ...
 {items.map(item => (
 <div key={item.id} className={`bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-3 rounded-xl flex flex-col shadow-sm transition hover:shadow-md ${editingItemId === item.id ? 'relative z-50' : ''}`}>
 ...
 <h6 className="font-bold text-sm text-slate-800 dark:text-[#f0ede6] truncate pr-2 flex items-center gap-2"><VegMarker item={item} />{item.name}</h6>
 <span className="font-mono text-xs font-bold text-amber-500 shrink-0">{formatINR(item.basePrice)}</span>
 </div>
 ...
 <Button variant="ghost" size="sm" onClick={() => startEditing(item)} className="p-1 !text-slate-400 hover:!text-amber-500">
 ...
 <div className="flex-1 min-w-0 flex flex-col justify-between">
 <div>
 <h6 className="font-bold text-xs text-slate-800 dark:text-[#f0ede6] truncate flex items-center gap-2"><VegMarker item={item} />{item.name}</h6>
 <span className="font-mono text-[10px] font-bold text-amber-500 shrink-0">{formatINR(item.basePrice)}</span>
 <div className="mt-1 flex items-center gap-2">
 <Badge variant="warning" className="flex items-center gap-1">
 <Clock className="w-2.5 h-2.5" /> {item.defaultPrepTimeMinutes} min
 </Badge>
